...
We interpret the main findings and scope the v1 work that should follow this internal v0 draft.

\subsection{What the results say about current LLMs}

\paragraph{Classical methods still win on clean formal tasks.} PC + greedy achieved the highest directed F1 and the lowest SHD on the ladder. Under the v0 assumptions, this is what theory suggests: PC is asymptotically correct and the greedy orientation heuristic is strong enough for small graphs. Today's LLMs are not drop-in replacements for classical structure-learning algorithms in settings where the SCM lies in their identifiability regime.

\paragraph{The precision--recall asymmetry is the main behavioral signal.} PC under-commits (precision $66.9$, recall $32.8$); LLMs over-commit (precision $35$--$46$, recall $20$--$30$). Neither failure is strictly better --- under-committers leave useful edges unresolved, over-committers inject wrong orientations --- and a benchmark that reports only F1 collapses the distinction. We recommend reporting precision and recall explicitly, and ACDB's scoring contract is designed to keep both visible.
...
\paragraph{Stats tools were not a free lunch.} Giving LLMs correlation, partial-correlation, and independence-test tools did not raise directed F1 above raw data. The stats variants ran longer trajectories, intervened less, and submitted fewer directed edges; their $100\%$ efficiency reflects abstention, not skill. Tool access alone is insufficient if the model does not know when to stop testing and commit.
