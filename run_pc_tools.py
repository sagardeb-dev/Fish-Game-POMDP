...
def build_pc_tools_system_prompt() -> str:
    return (
        ...
        "tools that expose PC's pipeline phases as composable primitives.\n"
        "\n"
        ...
        "direction. Observational data bounds you to the CPDAG (Markov-equivalence "
        "class); only interventions identify the DAG within that class.\n"
        "\n"
        "Submit_graph accepts both directed and undirected edges. An undirected "
        "edge means 'adjacency known, direction not resolved' — it is a legitimate "
        "answer when the evidence is genuinely ambiguous, scored at one SHD unit "
        "(the same cost as a reversal). A wrong directed edge also costs one SHD "
        "unit. Choose between leaving an edge undirected and committing to a "
        "direction based on the evidence you have.\n"
        "\n"
        "Tools.\n"
        "\n"
        "  pc_observational(alpha): runs PC on the observational dataset and "
        "returns the resulting CPDAG (compelled directed edges + undirected "
        "edges). One call gives you the entire observational ceiling for free. "
        "There is no reason to call this more than once per session.\n"
        "\n"
        "  meek_closure(directed_edges, undirected_edges): applies Meek rules "
        "R1–R3 to a partial DAG and returns the closure. Most useful after an "
        "intervention: a single new orientation often forces several others "
        "through the rules at no budget cost. To build the args after intervening "
        "on var, take pc_observational's compelled directed edges plus any new "
        "orientations you derived from mu_shift_vs_obs, and the remaining "
        "undirected edges (pc_observational's undirected set minus the ones var's "
        "shift just resolved). Call meek_closure between every intervene and the "
        "next decision — submitting without it typically leaves free orientations "
        "on the table.\n"
        "\n"
        "  intervene(var, value): consumes one unit of budget. Returns the "
        "post-intervention rows and the per-variable mean shift vs the "
        "observational baseline (mu_shift_vs_obs). For each undirected edge "
        "(var, other) incident to the intervened node, |mu_shift[other]| above "
        "~0.5 indicates var → other; smaller shifts indicate other → var. "
        "Targeting the highest-degree undirected node resolves the most edges "
        "per intervention.\n"
        "\n"
        "  submit_graph(directed, undirected, reasoning_summary): terminal.\n"
        "\n"
        "A natural workflow: call pc_observational once to get the CPDAG, then "
        "iterate intervene → meek_closure on undirected nodes (highest degree "
        "first) until either remaining_budget reaches 0 or no undirected edges "
        "remain, then submit. The intervene → meek_closure pairing is what makes "
        "interventions efficient; one without the other is rarely the right move "
        "unless the budget is exhausted. You can deviate from the surrounding "
        "structure if you have a reason — the tools are composable.\n"
        "\n"
        "Skeleton constraint: every pair (i, j) you submit (directed or "
        "undirected) should correspond to a pair pc_observational identified as "
        "adjacent. Submitting adjacencies PC didn't find adds extra-edge errors "
        "with no offsetting evidence.\n"
        "\n"
        "Exactly one tool call per turn. Do not request raw observational rows "
