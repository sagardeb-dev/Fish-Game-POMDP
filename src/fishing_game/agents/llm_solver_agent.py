...
from fishing_game.prompts.llm_solver import ESTIMATION_PROMPT


_REQUIRED_PATCH_KEYS = {
    "buoy_params",
    "equipment_inspection_params",
    "equipment_age_offset_factor",
...
class LLMSolverAgent:
    ...
    def _learn_from_history(self, env):
        ...
        missing_keys = sorted(_REQUIRED_PATCH_KEYS - set(patch))
        if missing_keys:
            raise ValueError(
                "LLM estimate missing required fields: "
                + ", ".join(missing_keys)
            )
    ...
    def act(self, env, obs, rng=None):
        ...
        observations = observation_bundle_to_pomdp_observations(obs)
        ...
        beliefs = belief_vector_to_decision_beliefs(self.pomdp, self.belief)
        p_storm = beliefs["storm_active"]
        p_equip = beliefs["equip_failure_active"]
        p_tide = beliefs["tide_high"]
