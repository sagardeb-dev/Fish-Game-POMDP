...
def test_validator_does_not_flag_moderate_transition_entropy_as_uniform():
    cfg = generate_config(curriculum_knobs(0.5), seed=42)
    cfg["sensor_zones_per_step"] = 2
    cfg["storm_transition"] = [[0.7, 0.3], [0.3, 0.7]]
    result = validate_config(cfg, strict=False)
    assert not any("storm_transition: very high row entropy" in warning for warning in result["warnings"])
